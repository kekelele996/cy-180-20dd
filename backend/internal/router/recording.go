package router

import (
	"log/slog"

	"github.com/gin-gonic/gin"
	"github.com/oralhistory/oralhistory/internal/config"
	"github.com/oralhistory/oralhistory/internal/constants"
	"github.com/oralhistory/oralhistory/internal/handler"
	"github.com/oralhistory/oralhistory/internal/middleware"
)

// RegisterRecordingRoutes 注册录音路由。
func RegisterRecordingRoutes(g *gin.RouterGroup, h *handler.RecordingHandler, cfg *config.Config, logger *slog.Logger) {
	group := g.Group("/recordings", middleware.Auth(cfg.JWTSecret, logger))
	{
		group.GET("", h.List)
		group.POST("", h.Create)
		group.GET("/:id", h.Get)
		group.PUT("/:id", h.Update)
		// 摘要审核流程：采访员（与管理员）提交，档案员（与管理员）批准/退回。
		group.PUT("/:id/summary/submit", middleware.RBAC(logger, constants.RoleInterviewer, constants.RoleAdmin), h.SubmitSummary)
		group.PUT("/:id/summary/approve", middleware.RBAC(logger, constants.RoleArchivist, constants.RoleAdmin), h.ApproveSummary)
		group.PUT("/:id/summary/reject", middleware.RBAC(logger, constants.RoleArchivist, constants.RoleAdmin), h.RejectSummary)
		group.POST("/:id/audio", h.UploadAudio)
		group.GET("/:id/audio", h.PlayAudio)
		group.DELETE("/:id", h.Delete)
	}
}
